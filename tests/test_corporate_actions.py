...
def test_prepare_corporate_actions_sorts_by_date():
    df = pd.DataFrame([
        ...
        {"code": "7203", "ex_date": "2024-01-15", "action_type": "CASH_DIVIDEND", "amount": 50.0},
    ])
...
def test_apply_split():
    ...
    updated, cash = _apply_corporate_actions([pos], [action], "2024-06-01", 1_000_000.0)
...
def test_dividend_long_receives_cash():
    pos = FakePosition(code="7203", side="BUY", qty=100, entry_price=5000.0)
    action = CorporateAction(code="7203", ex_date="2024-03-01", action_type="CASH_DIVIDEND", amount=50.0)
    updated, cash = _apply_corporate_actions([pos], [action], "2024-03-01", 1_000_000.0)
